...
- Router tests that cover HealthBoard must inject fake clients; never ping a live daemon.
- Routed health persistence must reuse `health.save_board` / `load_board` (same `health.json`).
