...
Ollama + LM Studio, ReAct tool loop, multi-LLM fallback router, SQLite vector memory, MCP stdio/HTTP/SSE with session ids + request cancel, local HTTP API, recipes, watcher, offline eval harness, **tool-result cache + telemetry**.
Offline-first.
